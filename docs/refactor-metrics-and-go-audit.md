...
- Extra catalog-merge check: `/tmp/aha-refactor-catalog-merge-extra.txt`
...
Median of 3 runs unless noted. Catalog merge also got a `-benchtime=5x -count=5` check because the `1x` run was noisy.
...
| Catalog merge, 4k refs/1k unique (`5x`, median of 5) | `837µs`, `705KB`, `2771 allocs` | `792µs`, `705KB`, `2771 allocs` | No regression; `1x` runs were noisy. |
| Local depot list, 250 refs | `1.15ms`, `374KB`, `2042 allocs` | `0.91ms`, `374KB`, `2042 allocs` | No regression. |
| Local depot append growing catalog | `2.57ms`, `~1.11MB`, `~2246 allocs` | `2.40ms`, `~1.12MB`, `~2268 allocs` | No material regression; tiny alloc increase buys no-replace publish semantics. |
...
| Depot local verify/list/append | same order as before | Still syscall/JSON/catalog dominated; atomic helper is not a visible hot frame. |

## Refactor changes completed

- Shared depot known-bundle normalization via `prepareKnownBundleRef`.
- Shared quick catalog-ref verification via `verifyCatalogRefs` with backend-specific existence checks.
- Shared catalog shard grouping/merge/write helpers for compact and repair paths.
...
- Shared search filter predicate construction for message/artifact SQL.
- Shared corpus FTS text predicates between verify, repair, and artifact triggers.
...
- Artifact FTS trigger now uses the same empty-body preview fallback as `verify` and `repair`; migration v7 recreates the trigger and rebuilds artifact FTS for existing corpora.

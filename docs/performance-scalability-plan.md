...
2. **Property-based performance invariants for metadata-heavy cases**: many trivial bundles, duplicates, many machines, and many catalogue shapes. These avoid storing huge datasets and assert complexity by operation/cardinality rather than wall-clock time.

The second layer is especially important for depot/status/refresh behaviour because the failure mode is often not a 100MiB object; it is years of tiny no-op snapshots (and, in the v1 depot, duplicate catalogue refs — depot v2's operation-count properties play the same role for pointer/manifest GETs and blob fetches).
...
Before changing an abstraction for performance, add a characterization gate that freezes the externally important behaviour and the performance invariant the new abstraction is supposed to improve. This is the minimum test net needed to reconsider internals freely. (The depot rows in the table below are v1-era records; depot v2 deleted those abstractions — catalogue `state_sha256` refs, `WriteWithInfo` bundle handoff, expected-SHA bundle ingest, and catalogue compaction — outright.)
...
|---|---|---|---|
| FTS `rowid` / indexed shadow-key verifier | Seed tiny corpora with matching rows, missing FTS rows, orphan FTS rows, artefact text rows, and repaired rows; assert `Verify`/`ReconcileFTS` reports are unchanged. | Tiny `EXPLAIN QUERY PLAN`/schema guard that verifier uses indexed row identity rather than scanning unindexed FTS columns; keep pathological verify benchmark for magnitude. | `BenchmarkPathologicalVerifyFTSJoinScaling` slope and `ns/op`; query-plan scan count. |
| Catalogue `state_sha256` refs | State signature equality remains `ManifestStateSHA256`-equivalent and ignores volatile bundle metadata; catalogue repair preserves bundle truth. | Fake-driver/PBT over refs with duplicates, many machines, stale ordering: unchanged refresh performs `0` `Fetch` calls and reads `0` old bundle bytes when state metadata exists. | Old bundle fetch count, old bundle bytes read, unchanged `refresh` latency. |
...
| `corpus.IngestBundleWithExpectedSHA` | Expected SHA mismatch fails before promotion; duplicate/corrupt bundle behaviour and JSON errors stay unchanged. | Fake/local operation test proves depot ingest performs one staging copy/hash and no separate pre-hash. | Bundle read passes per pending depot ingest; ingest wall time for large pending bundles. |
| Prepared/batched ingest writer | Corpus rows, conflicts, FTS rows, image/artefact blobs, and append-only behaviour match existing ingest on representative fixtures. | Small-model duplicate/conflict PBT plus package benchmark tracking `allocs/entry` and SQL calls/entry. | `allocs/op`, `allocs/entry`, SQL executions/entry, ingest `ns/op`. |
...
| Quick/deep depot verify | Deep verify still detects corrupt/missing/mismatched bundle bytes; quick verify reports metadata/catalogue problems only and says what it did not check. | Fake-R2 operation-budget tests: quick uses list/head metadata only; deep downloads/hashes; repair has explicit cost. | R2 GET/download count, bytes downloaded, local bytes hashed, verify latency. |
| Depot/status summaries or compaction | `depot ls`, pending ingest, repair, and status answers match the uncompacted catalogue model. | PBT over many trivial bundles/machines/shards; in-memory catalogue benchmarks before/after map-backed merge. | Catalogue rows scanned/read, JSON bytes parsed, refs merged/sec, status latency. |
...
(The v1 depot benchmarks — catalogue merge, large-catalogue list/verify — were deleted with the v1 bundle store; depot v2 paths are guarded by operation-count tests rather than benchmarks.)
...
Machine: Apple M2 Ultra, `go test ... -benchtime=1x -benchmem`. Numbers are directional, not release targets. This section is preserved as the pre-plan baseline; later results are in the next section and in `docs/performance-results.md`. The depot/catalogue rows measured the v1 bundle store.
...
| FTS repair | 5k messages + artefacts | `43ms` | In the baseline, full rebuild was cheaper than the old verifier; after rowid verification, repair remains an explicit maintenance path rather than the fast check. |
...
Machine: Apple M2 Ultra, `go test ... -benchtime=1x -benchmem` unless noted. These are not CI thresholds; they are before/after signals for the abstractions added by this plan. (v1-era capture: the depot merge and status/`BundleSHAs` rows measured v1 catalogue machinery that depot v2 has since deleted.)
...
| Depot merge | 1000 unique refs × 4 duplicates | `1.51ms`, `1.49MB`, `9.8k allocs` | Map-backed merge prevents quadratic duplicate behaviour and is used by repair/compact. |
...
Current examples (v2 operation-count/property tests replaced the deleted v1 catalogue tests):
...
- Depot profiles are syscall-heavy by design: local verify walks/stats/hashes files; catalogue append rewrites JSON atomically; deep R2 verify would add network download cost per object.

## How normal usage will degrade over time

| Growth axis | Current behaviour | Long-term risk |
|---|---|---|
...
| More tiny files | Per-file blob and manifest-entry overhead scales with file count, not just bytes (tar/header overhead now applies only to `export`). | Many subagent artefacts or small sessions create many small objects and manifest entries; R2's cheap op classes make this affordable, but manifests grow with file count. |
...
1. Store `state_sha256`, `manifest_sha256`, bundle size, captured timestamp, source counts, and implementation version in catalogue refs.
2. Make `findDepotBundleWithSameState` compare catalogue `state_sha256` first.
3. Fetch/read an old bundle only if the catalogue ref lacks state metadata or the catalogue needs repair.
4. Since this is pre-release, prefer a clean catalogue schema bump or a tested additive field strategy over compatibility shims.
5. Add fake-R2 operation-count and PBT model tests: unchanged refresh must not call `Fetch` when catalogue state metadata is present, regardless of duplicate refs, machine distribution, or catalogue ordering.
...
2. Thread expected SHA from depot catalogue into the ingest planner.
3. Let the planner's staging copy/hash be the authoritative compare point.
4. Remove the separate `archive.FileSHA256(path)` in `ingestFromDepot` for refs with validated catalogue keys.
...
Expected impact: lowers allocations and syscall/SQLite overhead for many tiny entries while preserving append-only conflict behaviour.
...
1. Split depot verification modes:
   - quick/default: marker, catalogue schema, key shape, duplicate refs, local object existence/head metadata;
   - deep: hash/read every bundle and validate manifest;
   - repair: rebuild catalogue from objects when local/deep enough data is available.
2. For R2, use object metadata/head where possible before downloads.
3. Store/refuse inconsistent catalogue fields (`bundle_sha256`, `key`, `size`, `manifest_sha256`, `state_sha256`).
...
6. Replace linear-scan bulk catalogue merging with map-backed merge for repair/compaction paths; keep append of one ref simple unless benchmarks show it dominates.
...
- PBT/operation-count invariants cover duplicate depot refs, map-backed catalogue merge, status metadata-only behaviour, duplicate ingest parse skipping, known file blob no-recompression, and no-op refresh state metadata.
- New catalogue refs carry `state_sha256`/`manifest_sha256`.
...
3. Catalogue summary files/local stale caches should be added only if real depots show raw catalogue scanning as a bottleneck; `compact` and map-backed merge are in place.

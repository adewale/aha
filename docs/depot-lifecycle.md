...
- **Provisioning lifecycle of a depot** — its backing store moves through
  **Uninitialized → Initialised → Populated**. `aha` provisions it for you on the
  first write, so you rarely run `aha archive init` by hand.
...
*Configuring* a depot moves the pointer — `aha archive init <addr>` (provision and
select) or `aha archive set-default <addr>` (switch to an already-initialised depot).
CLI addresses are always explicit: `r2:BUCKET` or `local:PATH`; bare values are
rejected rather than guessed. A
`--archive <addr>` flag is a one-off override: it targets a depot for a single
...
----------------------------------------
                aha archive init                  snapshot / refresh
...
--------------------------------------------------------------------
  config default ──(aha archive init <addr> | aha archive set-default <addr>)──▶ { local depot, r2 depot, … }
  --archive <addr>  = one-off override; never moves the pointer (may still auto-init <addr>)
  Only an Initialized depot may be selected; aha archive set-default refuses an uninitialized target.
```
...
1. **Uninitialized** — the backing store has no `aha-depot.json` marker **and
   ...
   the dashboard but never provisioned. `aha status` reports it reachable with
   `initialized: false` (`ok: true`) and an `aha archive init ...` next-action
   ...
   bundles via `aha archive download <bundle>`.
2. **Initialised** — the `aha-depot.json` marker is present
   (`schema: aha-depot/v2`, `layout: v2`). Reached by `aha archive init` **or
   implicitly by the first `aha archive upload` / `aha archive upload && aha archive download`**, which auto-create
...
4. **Degraded** (a sub-state of Initialised/Populated) — snapshot evidence
   ...
   identity check, or a referenced blob is absent. `aha archive verify` reports
...
default depot. `aha archive init` and `aha archive set-default` move it (only ever to an
Initialised depot); `--archive <addr>` overrides it for a single command. Switching
...
|---|---|---|---|---|
| `aha archive status r2:<bucket>` | read-only preflight | unchanged | yes, after local validation | Rejects malformed/placeholders before networking, reports the depot state, and emits exactly one safe next command. |
| `aha archive init <addr>` | Uninitialized → Initialised (idempotent) | sets default = `<addr>` | yes | Creates the dir/bucket if needed, writes the `aha-depot.json` marker, and for r2 persists the non-secret `depot.r2.account_id`. Re-running against an existing depot just connects. |
| `aha archive set-default <addr>` | requires Initialised | sets default = `<addr>` | yes | Switches the default to an already-initialised `<addr>`; refuses a reachable-but-uninitialized target and points at `aha archive init`. Persists r2 `account_id`. Creates nothing. |
| `aha archive upload` | Uninitialized → Initialised (auto) → Populated | unchanged | yes | Auto-initialises the target if needed, then pushes: uploads only blobs the parent snapshot does not carry, publishes the manifest, moves the pointer. Unchanged state is recognised from the pointer alone; it performs no blob/manifest/latest writes but repairs missing machine-index registration left by an interrupted first push. Does not touch the corpus and never reads another machine's namespace. |
| `aha archive upload && aha archive download` | same as `snapshot` → Populated | unchanged | yes | Push, then pull every machine's latest snapshot into the local corpus, fetching only unknown content. |
| `aha archive download` | reads Populated (no provisioning) | unchanged | yes | Pull-only: anti-entropy the corpus against every machine's latest snapshot. |
| `aha archive status` | reads Populated | unchanged | yes | Materializes a machine's latest snapshot as one portable v1 `bundle.tar.zst`. |
| `aha archive status` | reads Initialised/Populated | unchanged | yes | Lists each machine's latest snapshot (identity, capture time, file count). |
| `aha archive verify [--deep]` | reads (reports Degraded) | unchanged | yes | Quick: marker, index, pointers resolve, manifest identities, blob presence. `--deep`: verifies blob content and audits historical manifests. |
...
`--archive <addr>` on `snapshot` / `refresh` / `ingest` / `status` / `doctor` runs
that one command against `<addr>` without moving the default pointer. Because the
write commands auto-initialise their target, a one-off `snapshot --archive
...
```bash
aha init --acknowledge-raw-history   # writes config with the local depot default
aha archive upload && aha archive download                 # auto-initializes ~/.aha/depot, then snapshots + ingests
aha status                  # depot: local:~/.aha/depot ok=true, initialized=true
```

No `aha archive init` is needed for the local default — the first `refresh`
provisions it.
...
aha archive status r2:aha-depot  # read-only; prints one next command
aha archive init r2:aha-depot   # create/connect, write marker, set as default
aha archive upload && aha archive download --max-sessions 1  # bounded first push/pull
```

If doctor identifies a pre-v2 local corpus, `aha workspace repair --backup`
...
```bash
aha archive set-default r2:aha-depot
aha archive upload && aha archive download                   # shares history through R2
```

### Switch the default back

```bash
aha archive set-default local:~/.aha/depot   # back to local — both depots keep their data
aha archive set-default r2:aha-depot         # back to R2
```

## Maintenance

```bash
aha archive status --json                 # each machine's latest snapshot
aha archive verify --json             # quick: marker, index, pointers, manifests, blob presence
aha archive verify --deep --json      # also verify blob content + historical manifests (slow, explicit)
```

## Decommissioning

Depots are not deleted by `aha`. To stop using one, switch the default away
(`aha archive set-default <other>`) and then remove the backing store yourself: delete the

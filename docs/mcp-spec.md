...
| `incidents`   | `aha incidents`                      | `limit` (int, default 50); optional `state` (`unresolved`/`partial`/`resolved`), `project`, `source`, `machine`, `tool`                              | `[]corpus.Incident`                                   |
| `incident_trajectory` | (dashboard drill-in)         | `ref` (string, a resolving-success `msg:v1:` ref), optional `ordinal` (the path's `sample_ordinal`; required for multi-call resolving entries)       | `[]corpus.TrajectoryStep`                             |

...
| Failed pull preflight has no side effects | explicit CLI address parser rejects untyped destinations; `PreparedPull` requires a validated marker/index; opaque `CorpusDestination` accepts only a source-disjoint dedicated or existing corpus root | pull code cannot open SQLite, create lifecycle locks, or fetch blobs without both constructed capabilities | CLI regressions prove placeholder, ambiguous address, denied remote, and unrelated-directory failures create no corpus/lock/depot state |
...
| Legacy corpus rebuild always preserves recoverability | canonical symlink-resolved corpus identity; context-cancellable exclusive lifecycle capability; source-validated sibling staging; verified and recursively synced replacement; parent sync before/after one atomic directory exchange; no no-backup CLI variant | aliases cannot bypass locking or swap a symlink entry, callers cannot request destructive rebuild, promotion cannot occur before replacement verification/durability, and the configured root is never absent | deterministic contention/cancellation, symlink alias, durability-order/failure, build/verification/swap-failure, overlap, collision, backup-preservation, and CLI journey tests |

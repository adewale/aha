...
- `docs/outcome-weighting-spec.md` — design for the second half of the "sessions → skills" direction: ranking error clusters by the fix that actually worked. The shipped `aha clusters` surface ranks recurring tool-call *failures* (`is_error=1`) by recurrence and spread, but stops short of the fix; the `tool_invocations` table already stores *successes* too (every `OutcomeObserved` invocation), so resolution-path mining is a deterministic analysis layer — failure-episode assembly, Wilson-lower-bound path scoring, and top-K multi-path output for confounded clusters — over data already captured, with no new ingest or redaction boundary. Spec only; not yet implemented.

...
- `aha clusters --with-fixes` — outcome-weighted skill candidates: the second half of the "sessions → skills" direction. The plain `clusters` surface ranks recurring tool-call *failures* by recurrence and spread but stops short of the fix. `--with-fixes` mines single-session failure→fix arcs (new `failure_episodes` table, migrations 14–15, projected from the `tool_invocations` *successes* already stored) and ranks each resolved cluster by the resolution path that actually worked. Path confidence is a Wilson lower bound over the cluster's resolved-episode count, so a one-off fix (1/1) never outranks a repeatedly-confirmed one (3/4); confounded clusters surface their top-K competing paths rather than a single confidently-wrong recommendation; each candidate carries a resolution rate, a `tentative`/`established` tier, and a `sample_ref` to the resolving success. `failure_episodes` is a recomputable derived view (not append-only): each session's episodes are rebuilt — delete + reinsert — from its stored (already-redacted) invocations on every ingest, so a session resumed after its fix arrives correctly flips its abandoned episode to resolved instead of keeping a stale row (migration 15 drops the legacy append-only triggers and repairs existing corpora). No new ingest, no new redaction boundary. v1 is single-session and hard-signal-only (`is_error`/`exit_code`); a fix is only credited when the *same* command family that failed later succeeds. Surfaced across every read-only entry point: CLI (`aha clusters --with-fixes`), a dedicated `skill_candidates` MCP tool, `POST /api/skill_candidates` on the dashboard server, a new `resolved clusters — outcome-weighted fixes` section on the dashboard UI with click-to-read drill-in to the resolving success, and typed `SkillCandidate`/`ResolutionPath`/`SkillCandidatesArgs` + `aha().skill_candidates()` in the regenerated TypeScript client. MCP exposes it as a separate tool (not a `clusters` flag) so the chooser model picks "what worked" vs "what failed" by name. New `internal/corpus` units: `assembleEpisodes` (`episodes.go`), `wilsonLowerBound`/`spread`/`pathScore` (`scoring.go`), `failure_episodes` migration/backfill, and `SkillCandidates` (`outcomes.go`). See `docs/outcome-weighting-spec.md`.

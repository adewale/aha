...
- **Read-only MCP server** (`aha mcp`) — coding agents can call `search`, `read`, `clusters`, `status`, `verify`, `conflicts`, `corpus_size`, `skill_candidates`, and `doctor` as JSON-RPC tools. An agent can ask your history "have I asked this before?" or "which failures keep recurring?" via read-only tools.
...
- `mcp`: run a read-only stdio MCP server over the corpus so coding agents can call `search`, `read`, `clusters`, `status`, `verify`, `conflicts`, `corpus_size`, `skill_candidates`, and `doctor` as JSON-RPC tools. `--dry-run` opens the corpus, registers the tools, prints a one-line summary, and exits — a pre-flight check for host wiring. See `docs/mcp-spec.md`.

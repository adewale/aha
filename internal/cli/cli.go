...
func Registry() map[string]Command {
	return map[string]Command{
		...
		"serve":     {Name: "serve", Usage: "aha serve [--addr HOST:PORT] [--allow-remote] [--allowed-hosts H1,H2] [--timeout DUR] [--config PATH] [--repo DIR]", Flags: []string{"--addr", "--allow-remote", "--allowed-hosts", "--config", "--corpus", "--repo", "--timeout"}, Examples: []string{"aha serve", "aha serve --addr 127.0.0.1:18428"}, JSONSchema: "http://HOST:PORT/api/{search,read,status,verify,conflicts,corpus_size,doctor}", Docs: "run a read-only local dashboard over the corpus on loopback", Run: cmdServe},
	}
}

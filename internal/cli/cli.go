...
func Registry() map[string]Command {
	return map[string]Command{
		...
		"clusters":  {Name: "clusters", Usage: "aha clusters [--repo DIR] [--limit N] [--json]", Flags: []string{"--config", "--corpus", "--json", "--limit", "--repo"}, Examples: []string{"aha clusters --json", "aha clusters --limit 20"}, JSONSchema: "array<object{tool_name,command_family,error_signature,count,distinct_sessions,distinct_projects,first_seen,last_seen,sample_command,sample_error,sample_ref,score}>", Docs: "rank recurring tool-call failure clusters as skill-candidate signals", Run: cmdClusters},

...
func Registry() map[string]Command {
	return map[string]Command{
		...
		"depot":     {Name: "depot", Usage: "aha depot <setup|init|use|ls|verify> [DEPOT] [--progress MODE] [--json] [--deep]", Flags: []string{"--config", "--deep", "--json", "--progress"}, Examples: []string{"aha depot setup r2:aha-depot --json", "aha depot init local:~/.aha/depot", "aha depot init r2:aha-depot", "aha depot use r2:aha-depot", "aha depot ls --json", "aha depot verify --deep"}, JSONSchema: "object|array", Docs: "preflight R2 with one safe next action, initialise a depot, switch the default, list snapshots, or verify content", Run: cmdDepot, RunContext: runDepotContext},

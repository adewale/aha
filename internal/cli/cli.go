...
func Registry() map[string]Command {
	return map[string]Command{
		...
		"clusters":  {Name: "clusters", Usage: "aha clusters [--repo DIR] [--limit N] [--with-fixes] [--json]", Flags: []string{"--config", "--corpus", "--json", "--limit", "--repo", "--with-fixes"}, Examples: []string{"aha clusters --json", "aha clusters --limit 20", "aha clusters --with-fixes"}, JSONSchema: "default: array<object{tool_name,command_family,error_signature,count,distinct_sessions,distinct_projects,first_seen,last_seen,sample_command,sample_error,sample_ref,score}>; with --with-fixes: array<object{tool_name,command_family,error_signature,episodes,resolved,resolution_rate,paths:array<object{families,support,distinct_sessions,distinct_projects,confidence,sample_ref}>,score,tier}>", Docs: "rank recurring tool-call failure clusters as skill-candidate signals; --with-fixes ranks resolved clusters by the resolution path that fixed them", Run: cmdClusters},
		...
		"depot":     {Name: "depot", Usage: "aha depot <init|use|ls|verify|compact> [DEPOT] [--json] [--repair] [--deep]", Flags: []string{"--config", "--deep", "--json", "--repair"}, Examples: []string{"aha depot init local:~/.aha/depot", "aha depot init r2:aha-depot", "aha depot use r2:aha-depot", "aha depot ls --json", "aha depot verify --deep", "aha depot verify --repair", "aha depot compact --json"}, JSONSchema: "object|array", Docs: "initialize a depot, switch the default depot, list, verify, or compact a bundle depot", Run: cmdDepot},

...
func TestVerifyReportsSeededCorpusDrift(t *testing.T) {
	...
	if _, err := store.DB.Exec(`insert into snapshots(manifest_sha256,machine_id,captured_at,ingested_at,manifest_json) values('` + strings.Repeat("a", 64) + `','m','2026','2026','{}')`); err != nil {
		t.Fatal(err)
	}
	...
	for _, code := range []string{"orphan_messages", "orphan_fts_messages"} {
